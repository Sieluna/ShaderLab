...
impl HomePage {
    ...
    pub fn view(&self) -> Element<Message> {
        ...
        let featured_grid = container(row![].spacing(20).padding(20)).width(Length::Fill);

        // Category section
        let category_title = container(
            row![
                text(format!("{} Content", self.selected_category)).size(24),
                horizontal_space(),
                button("View More")
                    .padding([8, 16])
                    .style(button::secondary)
        ...
        let category_notebooks: Vec<_> = self
            .notebooks
            .iter()
            .filter(|n| n.category == self.selected_category)
            .collect();

        let category_grid = row![].spacing(20).padding(20).width(Length::Fill);

        let category_grid = category_notebooks
            .iter()
            .fold(category_grid, |row, notebook| row.push(notebook.view()));
