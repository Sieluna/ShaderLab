...
pub struct MenuBarOverlay<'a, 'b, Message, Theme, Renderer>
where
    Theme: Catalog,
    Renderer: renderer::Renderer,
{
    ...
    pub roots: &'b mut [Item<'a, Message, Theme, Renderer>],
    pub init_bar_bounds: Rectangle,
    pub init_root_bounds: Vec<Rectangle>,
    pub check_bounds_width: f32,
    pub draw_path: &'b DrawPath,
    pub scroll_speed: ScrollSpeed,
    pub class: &'b Theme::Class<'a>,
}
...
where
    ...
    Renderer: renderer::Renderer,
{
    pub fn overlay_element(self) -> overlay::Element<'b, Message, Theme, Renderer> {
    ...
    Renderer: renderer::Renderer,
{
    ...
    fn draw(
        ...
        let menu_layouts_layout = lc.next().unwrap();
    ...
    fn operate(
        ...
        fn rec<'a, 'b, Message, Theme: Catalog, Renderer: renderer::Renderer>(
            ...
            operation.container(None, menu_layout.bounds(), &mut |operation| {
                menu.items
                    .iter()
                    .zip(menu_tree.children.iter_mut())
    ...
    fn on_event(
        ...
        fn rec<'a, 'b, Message, Theme: Catalog, Renderer: renderer::Renderer>(
            ...
            let slice_layout = mc.next().unwrap();
    ...
    fn overlay<'c>(
        ...
        let menu_layouts_layout = lc.next()?;
        let mut menu_layouts = menu_layouts_layout.children();
        let active_root = &mut self.roots[active];
        let active_tree = &mut self.tree.children[active];
