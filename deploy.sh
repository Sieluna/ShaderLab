...
get_latest_release() {
  ...
  local download_url
  if ! download_url=$(echo "${release_info}" | jq -r ".assets[] | select(.name | contains(\"${BINARY_NAME}.zip\")) | .browser_download_url"); then
  ...
  local checksum_url
  checksum_url=$(echo "${release_info}" | jq -r ".assets[] | select(.name | contains(\"${BINARY_NAME}.zip.sha256\")) | .browser_download_url" || echo "")
...
cmd_db_backup() {
  ...
  [[ ! -f "$DB_FILE" ]] && { log_warn "Database file not found, skipping backup"; return 0; }
...
main() {
  ...
  case "${1:-help}" in
    ...
    db:list)        
      ...
      ;;
    db:restore)     shift; cmd_db_restore "${1:-}" ;;
    help)           cmd_usage ;;
    *)              cmd_usage; exit 1 ;;
  esac
}
